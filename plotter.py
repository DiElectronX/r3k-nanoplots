...
def get_data(cfg, cache):
    ...
    data = {key : raw_data[key].to_numpy() if raw_data[key].ndim==1 else raw_data[key][:,0].to_numpy() for key in cfg['branches']} 
    cache.append({'inputs':cfg['inputs'],'cuts':cfg['cuts'],'branches':cfg['branches'],'data':data})

    return data

def make_plot(data, ax, cfg):
    for i, branch in enumerate(cfg['branches']):
        weights = np.ones_like(data[branch])/len(data[branch]) if cfg.get('norm', False) else np.ones_like(data[branch])
...
def plot_inputs(args):
    ...
    for plot, plot_cfg in plots_cfg['plots'].items():
        if plot_cfg.get('skip',False): continue
